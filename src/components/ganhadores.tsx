    ...
    <div className={styles.ganhadores_container}>
      ...
      <div className={styles.ganhadores}>
        {ganhadores?.map(
          (ganhador, index) =>
            index <= 4 && (
              <div className={styles.ganhador} key={ganhador.id}>
                <p className={styles.name}>{ganhador.user}</p>
                <div
                  className={`${styles.premio} ${ganhador.award === 6 ? styles.premioBest : ''}`}
                >
                  <Image
                    alt="imagem do premio ganho"
                    src={`/item${ganhador.award}.svg`}
                    width={100}
                    height={140}
                  />
                </div>
